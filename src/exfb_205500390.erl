...
%%% EXTRAS
%% boolGenerator - creates NumOfEquations Equations, of NumOfVars variables
booleanGenerator(NumOfVars,NumOfEquations) -> booleanGenerator(NumOfVars, NumOfEquations, []).
booleanGenerator(_, 0, List) -> List;		% done
booleanGenerator(N, E, List) -> 	% creating new list with new bool function, M+5 to bound runtime
  BF = randomBoolFunc(N,N+1),
	booleanGenerator(N,E-1,[BF|List]).	


%% randomBoolFunc - creating a random boolean function with max recursive clause M, N variables
randomBoolFunc(N,0) -> randomVar(N);   % do not continue!
randomBoolFunc(N,M) ->                % M is the maximum recursive clause, N is the number of variables
  case rand:uniform(16) of
    1 -> {randomOp(), {randomBoolFunc(N,M-1),randomBoolFunc(N,M-1)}};
    2 -> {randomOp(), {randomVar(N),randomBoolFunc(N,M-1)}};
    3 -> {randomOp(), {randomBoolFunc(N,M-1), randomVar(N)}};
    4 -> {randomOp(), {randomVar(N), randomVar(N)}};

    5 -> {randomOp(), {{'not',randomBoolFunc(N,M-1)},randomBoolFunc(N,M-1)}};
    6 -> {randomOp(), {{'not',randomVar(N)},randomBoolFunc(N,M-1)}};
    7 -> {randomOp(), {{'not',randomBoolFunc(N,M-1)}, randomVar(N)}};
    8 -> {randomOp(), {{'not',randomVar(N)}, randomVar(N)}};

    9 -> {randomOp(), {randomBoolFunc(N,M-1),{'not',randomBoolFunc(N,M-1)}}};
    10 -> {randomOp(), {randomVar(N),{'not',randomBoolFunc(N,M-1)}}};
    11 -> {randomOp(), {randomBoolFunc(N,M-1), {'not',randomVar(N)}}};
    12 -> {randomOp(), {randomVar(N), {'not',randomVar(N)}}};

    13 -> {randomOp(), {{'not',randomBoolFunc(N,M-1)},{'not',randomBoolFunc(N,M-1)}}};
    14 -> {randomOp(), {{'not',randomVar(N)},{'not',randomBoolFunc(N,M-1)}}};
    15 -> {randomOp(), {{'not',randomBoolFunc(N,M-1)}, {'not',randomVar(N)}}};
    16 -> {randomOp(), {{'not',randomVar(N)}, {'not',randomVar(N)}}}
  end.


%% randomVar - randomize a variable
randomVar(NumOfVars) ->
  R = lists:seq(1, NumOfVars),
  List = [list_to_atom(lists:flatten(io_lib:format("x~B", [X]))) || X <- R],  % list of {xi}
  lists:nth(rand:uniform(NumOfVars), List).    % getting the random index element
